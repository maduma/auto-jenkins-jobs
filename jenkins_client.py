...
PipelineState = collections.namedtuple(
    'PipelineState',
    'is_folder_exists, is_pipeline_exists is_folder_updated is_pipeline_updated',
    defaults=[True] * 4,
    )
...
def is_job_xml_updated(name, template):
    ...
    return job_version == get_version(template_xml)
...
def create_pipeline_xml(
    ...
    xml = xml.format(
        ...
        encrypted_trigger_token=encrypt(project.trigger_token),
    )
    return xml
